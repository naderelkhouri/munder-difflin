...
import {
  rmSync, existsSync, readFileSync, readdirSync, statSync, writeFileSync,
...
import {
  readConfig, writeConfig, resetConfig, onConfigWritten, ensureClaudePermissionsAccepted,
...
function teardownForChangeHome(): void {
...
function recoverAfterMoveFailure(): void {
...
function trackOnboardingCompleted(wasOnboarded: boolean, next: HarnessConfig): void {
...
registerModularIpc({
  ptyManager,
  roster,
  onTeardownPty: teardownPty,
  configServices: {
    hive,
    ptyManager,
    bootstrapHiveServices,
    teardownForChangeHome,
    recoverAfterMoveFailure,
    setAllowQuit: (allow: boolean) => {
      allowQuit = allow;
    },
...
// ─── IPC: integrations & provider keys (delegated to ./ipc/integrationsIpc) ─

// ─── IPC: config (delegated to ./ipc/configIpc) ─────────────────────────────
